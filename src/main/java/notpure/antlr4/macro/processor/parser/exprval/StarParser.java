 ...
 */
public final class StarParser implements ExpressionParser<ExpressionValue> {
    @Override
    public ExpressionValue parse(TokenParserIterator it) throws ParserException {
        return new ExpressionValue(ExpressionValueType.STAR, it.next().getValue());
    }

    @Override
    public boolean validate(TokenParserIterator it) {
        return it.hasNext() && it.peek().nameEquals(TokenDefinition.ASTERISK);
    }
}
